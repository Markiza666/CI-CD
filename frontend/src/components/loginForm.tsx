...
            });
            
            const { token } = response.data; // <-- Vi behöver bara token här

            // 1. Skicka ENDAST token till AuthContext. User-objektet genereras internt.
            login(token); // 🛑 FIX: Tog bort det andra argumentet (user)
